...
class TrainConfig(pydantic.BaseModel):
    ...
    # Names
    checkpoint_path: Optional[str] = "checkpoints/trm_image"
...
def create_model(config: TrainConfig, train_metadata: ROCOv2DatasetMetadata, world_size: int):
    model_cfg = dict(
        ...
        puzzle_emb_ndim=0,
        puzzle_emb_len=0, # Fix dimension mismatch
        
        # TRM specific
        H_cycles=1,
        # M is not usually in standard TRM unless added by us
        L_cycles=1,
        H_layers=2,
        L_layers=2,
        hidden_size=512,
        ...
        # Triple loop added? In TRM.py it seems to support it?
        # Checked trm.py in view_file Step 761, it has H_cycles/L_cycles but NO use_triple_loop or M_cycles in standard config.
        # But wait, trm_image.py inherited from TinyRecursiveReasoningModel_ACTV1Config which I defined in trm.py.
        # Step 761 content does NOT show triple loop in TinyRecursiveReasoningModel_ACTV1Config.
        # So standard TRM is H-L.
...
def main():
    ...
    print("Starting TRM Image training (ResNet18)...")
    all_metrics = []
