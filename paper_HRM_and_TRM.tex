...
% -------------------- ABSTRACT --------------------
\begin{abstract}
Recent advancements in Hierarchical Reasoning Models (HRM) have demonstrated strong capabilities in complex algorithmic and abstract reasoning tasks by mimicking multi-timescale cognitive processes \cite{wang2025hrm}. In this work, this architecture is extended to the domain of medical image captioning, leveraging the large-scale ROCOv2 dataset \cite{roco2020dataset}. We introduce a unified \textbf{ImageHRM} architecture that integrates a high-performance visual backbone directly into the recurrent reasoning modules of HRM. We evaluate three encoder variants: the foundational ResNet18 \cite{he2016resnet}, the multi-scale Swin Transformer \cite{liu2021swin}, and the highly integrated FuseLIP multimodal encoder \cite{schlarmann2025fuselip}. Furthermore, the standard Dual-Loop reasoning mechanism is extended to a novel \textbf{Triple-Loop Reasoning (H-M-L)} structure, establishing a three-tiered abstraction hierarchy (High-Middle-Low) specifically designed to capture the structural and semantic depth required for complex radiological reports \cite{reportstructure2020}. This architecture allows for sustained, deep latent reasoning that is essential for synthesizing global diagnostic impressions from clustered low-level findings, all while maintaining a constant memory footprint independent of sequence length. Our experiments demonstrate the significant efficacy of this reasoning-based generation approach, showing that the Triple-Loop ImageHRM, particularly when paired with an early-fusion backbone, significantly surpasses conventional fixed-depth baselines in structural coherence and accuracy.
\end{abstract}
...
c:\Users\user\Downloads\2510.04871v1 (1).pdf
...
Models such as PubMedCLIP fine-tune the CLIP architecture on the ROCO dataset, while MedCLIP \cite{wang2021medclip} utilizes advanced backbones like BioClinicalBERT and the Swin Transformer \cite{liu2021swin}, pre-trained on datasets like MIMIC-CXR \cite{mimiccxr2019}. The Swin Transformer \cite{liu2021swin} is particularly effective as a visual encoder in the medical domain due to its hierarchical structure and shifted window attention mechanism, which allows it to efficiently capture multi-scale pathological features, from local textures to global anatomical structures. This architectural design is necessary to ground the reasoning core in rich, clinically relevant visual evidence. The competitive landscape, which includes models like BiomedCLIP, demonstrates that while large-scale pre-training offers impressive performance, explicit reasoning structures like ImageHRM offer a potent alternative for synthesizing deep logical connections required for highly structured reports.
