...
def load_model(checkpoint_path: str):
    ...
    # Recreate config
    # We should ideally load config from file but we hardcode for now or infer
    # Assuming TRMImage default config + changes
    
    # Metadata for model creation (dummy values if we don't have dataset loaded yet, or load dataset first)
    # Let's load dataset first to get vocab size etc.
    return None

def main():
    checkpoint_dir = "checkpoints/trm_image"
    ...
    # Load weights
    # We wrapped it in ACTLossHead in training, so state_dict keys might have `model.` prefix if we saved `train_state.model` (which helps ACTLossHead wrapper)
    # In training: `train_state.model` is ACTLossHead.
    # We saved `train_state.model.state_dict()`.
    # ACTLossHead has `self.model`.
    # If we load into TRMImage directly, we need to strip `model.` prefix, OR wrap here too.
    
    # Let's wrap to be consistent
    ...
    with torch.no_grad():
        for i, batch in tqdm.tqdm(enumerate(loader), total=min(len(loader), max_eval)):
            ...
            # TRMImage_Inner expects images on same device
            # Wrapped in ACTLossHead -> model -> inner
            if hasattr(model.model.inner, "vision_model"):
                vd = next(model.model.inner.vision_model.parameters()).device
                images = images.to(vd)
            
            img_tensor = images # [1, 3, 224, 224]
            bos_id = 2
            eos_id = 1
            pad_id = 0
            gen_toks = [bos_id]
            ...
            for _ in range(64):
                ...
                bat = {
                    ...
                    "labels": padded_inp.clone() # Dummy
                }
                
                carry = model.initial_carry(bat)
                
                # Forward to get next token logic
                # Loop until halted? ACT wrapper handles loop internally?
                # In training: `train_state.model(carry=carry, batch=bat)` returns all outputs.
                # ACTLossHead forward returns `(new_carry, outputs, (q_halt, q_cont))`.
                # We need `outputs["logits"]`.
                
                # But wait, ACTLossHead forward executes the recursive loop?
                # TRMImage.forward executes ONE step if `halt_max_steps` logic is handled?
                # No, TRMImage.forward executes a single step of "recursive loop" if we look closely at `trm.py`?
                # Check `trm.py` forward.
                # `for _H_step` loops are INSIDE `forward`.
                # So ONE call to `model(batch)` computes the full pass (unless doing TBPTT or something, but here it loops H*L times).
                # Wait, `trm.py` forward loops `H_cycles` times.
                # So it produces the final output.
            ...
            ref_txt = "".join([chr(t) for t in ref_ids if t > 2]) # filter special
